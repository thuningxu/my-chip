...
//
// And a SECOND exhaustive claim, added because rtl/amx_fp8.v now spends flops on
// it: y[15:0] is zero on all 262,144 inputs, which is what makes that design's
// PIPE=1 product register 16 bits wide instead of 32. See the check at the end of
// the sweep -- it names the file it protects, because a failure there is a bug
// there and not here.
//
//-------------------------------------------------------- CROSS-LANGUAGE TIE
...
module tb_fp8_mul;
    ...
    integer inexact    = 0;
    integer shown      = 0;
    // Counts inputs whose product has ANY of its low 16 bits set. See the
    // low-bits check after the sweep for what depends on this being zero.
    integer lowbits    = 0;
    ...
    initial begin
                ...
                for (xi = 0; xi < 256; xi = xi + 1)
                    for (yi = 0; yi < 256; yi = yi + 1) begin
                        ...
                        if (!prod_exact(fp8_ref(xi[7:0], fa_i[0]),
                                        fp8_ref(yi[7:0], fb_i[0])))
                            inexact = inexact + 1;
                        // Checked on the DUT's OWN output, not on the model's:
                        // amx_fp8 registers these 32 bits, and it is the RTL that
                        // has to have zero low bits, not a model that agrees with
                        // it. See the low-bits check after the sweep.
                        if (y_out[15:0] !== 16'b0) lowbits = lowbits + 1;
                        sum = sum + y_out;
                    end
        ...
        // WHAT rtl/amx_fp8.v's 16-BIT PRODUCT REGISTER RESTS ON, converted from an
        // argument into an exhaustive proof. At PIPE=1 amx_fp8 registers only
        // prod[31:16] and re-supplies 16'b0 on the far side (see g_prod_reg), which
        // halves that cut from 32,768 flops to 16,384. The justification is that
        // fp8_mul's normal_y is {sgn, e_fld, nrm[6:0], 16'd0} and that every
        // special it can return -- 0x7FC00000, {sgn,8'hFF,23'd0}, {sgn,31'd0} --
        // also has zero low bits, because a product of two 4-bit significands has
        // exactly 7 fraction bits. Reading the RTL and believing that is not the
        // same as checking it on all 262,144 inputs, so it is checked here.
        //
        // IF THIS EVER FAILS, the register in rtl/amx_fp8.v is silently dropping
        // product bits -- a wrong answer in the array with nothing wrong in the
        // multiplier -- and the two must be fixed together. Widen it to 32 bits, or
        // stop claiming the truncation is lossless.
        if (lowbits != 0) begin
            $display("  [FAIL] %0d products had y[15:0] != 0 -- rtl/amx_fp8.v's 16-bit product register DROPS BITS",
                     lowbits);
            fail_count = fail_count + 1;
        end else
            $display("  [PASS] all 262144 products have y[15:0]==0 (amx_fp8's 16-bit product register is lossless)");
